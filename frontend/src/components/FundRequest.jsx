    ...
    <div className="min-h-screen bg-gradient-to-br from-amber-50 to-orange-50 py-8 px-4">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-12">
          <div className="inline-flex items-center justify-center w-20 h-20 bg-gradient-to-r from-amber-500 to-orange-600 rounded-2xl shadow-xl mb-6">
          ...
          <p className="text-xl text-gray-700 max-w-3xl mx-auto">
            Support patients in need and help them access life-saving medical treatments through your generous donations
          </p>
        </div>

        {/* Stats Overview */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <div className="bg-white rounded-2xl shadow-xl p-6 border border-amber-100 transform hover:-translate-y-1 transition-all duration-300">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-amber-600 text-sm font-semibold mb-2">Total Requests</p>
          ...
          <div className="bg-white rounded-2xl shadow-xl p-6 border border-red-100 transform hover:-translate-y-1 transition-all duration-300">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-red-600 text-sm font-semibold mb-2">Urgent Needs</p>
                <h3 className="text-3xl font-bold text-gray-900">{stats.urgent}</h3>
                <p className="text-gray-500 text-sm mt-1">Require immediate help</p>
              </div>
          ...
          <div className="bg-white rounded-2xl shadow-xl p-6 border border-green-100 transform hover:-translate-y-1 transition-all duration-300">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-gray-500 text-sm mt-1">Successfully funded</p>
              </div>
          ...
          <div className="bg-white rounded-2xl shadow-xl p-6 border border-blue-100 transform hover:-translate-y-1 transition-all duration-300">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-blue-600 text-sm font-semibold mb-2">Total Raised</p>
                <h3 className="text-3xl font-bold text-gray-900">₹{stats.totalRaised.toLocaleString()}</h3>
                <p className="text-gray-500 text-sm mt-1">Community contributions</p>
              </div>
              ...
              <div className="p-6">
                <div className="space-y-4 mb-6">
                  ...
                  <div className="bg-amber-50 rounded-xl p-4 border border-amber-200">
                    <p className="text-sm text-amber-800 font-semibold mb-1">Medical Purpose</p>
                  ...
                  <div className="flex items-center space-x-3 text-sm text-gray-600">
                    ...
                    <div>
                      <p className="font-semibold">{request.hospital?.name || 'Hospital'}</p>
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="font-bold text-amber-600">
                        {getProgressPercentage(request.amountCollected, request.amountRequired).toFixed(1)}%
                      </span>
                    </div>
                    <div className="w-full bg-gray-200 rounded-full h-3">
                      <div
                        className={`h-3 rounded-full bg-gradient-to-r ${getProgressColor(
                          getProgressPercentage(request.amountCollected, request.amountRequired)
                    ...
                    <div className="flex justify-between text-sm text-gray-600">
                      <span>₹{request.amountCollected.toLocaleString()} raised</span>
                      <span>₹{request.amountRequired.toLocaleString()} goal</span>
                    </div>
                  </div>
                </div>
          ...
          <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center p-4 z-50 backdrop-blur-sm">
            <div className="bg-white rounded-3xl max-w-md w-full p-8 shadow-2xl transform animate-scale-in">
              <div className="text-center mb-6">
                ...
                <h3 className="text-2xl font-bold text-gray-900">Support {selectedRequest.patientName}</h3>
                <p className="text-gray-600 mt-2">Your donation can make a difference</p>
              </div>
              
              <div className="space-y-4 mb-6 bg-gray-50 rounded-2xl p-4">
                <div className="flex justify-between items-center">
                  <span className="text-gray-600">Purpose:</span>
                  <span className="font-semibold">{selectedRequest.purpose}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-gray-600">Goal Amount:</span>
                  <span className="font-bold text-amber-600">₹{selectedRequest.amountRequired.toLocaleString()}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-gray-600">Already Raised:</span>
                  <span className="font-bold text-green-600">₹{selectedRequest.amountCollected.toLocaleString()}</span>
                </div>
                {selectedRequest.hospital?.name && (
                  <div className="flex justify-between items-center">
                    <span className="text-gray-600">Hospital:</span>
                    <span className="font-semibold">{selectedRequest.hospital.name}</span>
                  </div>
                )}
              </div>
        ...
        {filteredRequests.length === 0 && !loading && (
          <div className="text-center py-16 bg-white rounded-2xl shadow-lg">
            ...
            <h3 className="text-2xl font-bold text-gray-900 mb-3">No Fund Requests Found</h3>
        ...
        {loading && (
          <div className="flex flex-col items-center justify-center py-16">
            ...
            <p className="text-gray-600 text-lg font-medium">Loading fund requests...</p>
          </div>
        )}
      </div>
