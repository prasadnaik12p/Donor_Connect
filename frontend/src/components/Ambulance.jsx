...
const Ambulance = ({ user }) => {
    ...
    if (user?.id) {
      ...
      socketService.onEmergencyAccepted((data) => {
        ...
        setSelectedAmbulance({
          id: data.ambulanceId,
          name: data.ambulanceName,
          phone: data.ambulancePhone,
  ...
  const getCurrentLocation = () => {
    ...
    if (navigator.geolocation) {
        ...
        (error) => {
          ...
          switch (error.code) {
            case error.PERMISSION_DENIED:
              errorMessage +=
                "Please allow location access or use manual search.";
              break;
            case error.POSITION_UNAVAILABLE:
              errorMessage +=
                "Location information unavailable. Use manual search.";
              break;
            case error.TIMEOUT:
      ...
      setLocationError(
        "Geolocation is not supported by this browser. Use manual search.",
      );
    }
  };
    ...
    try {
      ...
      if (response.data.success) {
        ...
        if (response.data.ambulances.length === 0) {
          alert(
            "No ambulances found in your area. Try expanding search or using manual location.",
          );
        }
      } else {
      ...
      alert(
        "Failed to fetch nearby ambulances: " +
  ...
  const createEmergencyRequest = async () => {
    ...
    if (searchType === "auto") {
      if (!location.lat || !location.lng) {
        ...
        if (!location.lat || !location.lng) {
          alert(
            "Unable to get your location. Please use manual search or allow location access.",
          );
          return;
        }
      }
...
Stay calm and wait for the ambulance to arrive. Keep your phone accessible.
          ...
          if (checkCount === 40) {
            // 2 minutes
            alert(
              "⚠️ Still searching for available ambulances. If no ambulance responds within 1 minute, please try again or call emergency services directly.",
            );
          }
        }
...
We apologize for the inconvenience.
  ...
  const handleCoordinatesExample = () => {
    ...
    alert(
      'Example coordinates for Bangalore added! Format: "latitude longitude"',
    );
  };
                ...
                <p className="text-red-900 text-sm mb-2">
                  ...
                  {emergencyStatus === "assigned" &&
                    currentEmergency.assignedAmbulance && (
                      <span>
                        {" "}
                        • Assigned to: {currentEmergency.assignedAmbulance.name}
                      </span>
                    )}
                </p>
                ...
                {emergencyStatus === "searching" && (
                  <div className="mt-2">
                    ...
                    <p className="text-red-100 text-xs text-center">
                      Searching... {Math.round(getProgressPercentage())}% • Time
                      remaining: {Math.round(((60 - checks) * 3) / 60)} min
                    </p>
                  </div>
                )}
              </div>
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-gray-700">
                      Showing all available ambulances regardless of location
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-3">
              <div className="text-center sm:text-left">
                ...
                <p className="text-green-100 text-sm sm:text-base">
                  ...
                  {searchType === "coordinates" &&
                    "Available near specified coordinates"}
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      <p className="text-gray-500 text-sm">
                        {ambulance.type || "Medical Ambulance"}
                      </p>
                    </div>
                  </div>
                  <span
                    className={`px-3 py-1 rounded-full text-xs font-semibold border ${getStatusColor(ambulance.status)}`}
                  >
                ...
                <div className="space-y-3 mb-6">
                  ...
                  <div className="flex items-center space-x-3 text-gray-600">
                    ...
                    <span className="flex-1">
                      ...
                      {ambulance.hospital?.name || "Independent"}
                    </span>
                  </div>
                  <div className="flex items-center space-x-3 text-gray-600">
                    <span className="w-5">📍</span>
                    <span className="flex-1">
                      ...
                      {ambulance.hospital?.city || ambulance.city || "Unknown"}
                    </span>
                  </div>
                </div>
                  ...
                  <div className="space-y-3">
                    ...
                    {!emergencyCreated && (
                      <button
                        onClick={() => {
                          alert(
                            `Direct emergency alert sent to ${ambulance.name}! They will contact you shortly.`,
                          );
                ...
                {ambulance.status !== "available" && (
                  <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-4 text-center">
                    ...
                    <p className="text-yellow-700 text-sm mt-1">
                      {ambulance.status === "onDuty"
                        ? "This ambulance is currently serving another emergency"
                        : "This ambulance is not available at the moment"}
                    </p>
                  </div>
                )}
              </div>
          ...
          <div className="text-center py-16 bg-white rounded-2xl shadow-xl">
            ...
            <h3 className="text-2xl font-bold text-gray-900 mb-3">
              {searchType === "auto"
                ...
                : searchType === "manual"
                  ? "No ambulances found in this city"
                  : searchType === "coordinates"
                    ? "No ambulances found near these coordinates"
                    : "No ambulances currently available"}
            </h3>
            <p className="text-gray-600 max-w-md mx-auto mb-6">
              Try changing your search criteria, expanding the search radius, or
        ...
        {loading && (
          <div className="text-center py-16 bg-white rounded-2xl shadow-xl">
            ...
            <h3 className="text-xl font-semibold text-gray-900 mb-2">
              {searchType === "auto"
                ? "Searching for ambulances near you..."
                : searchType === "manual"
                  ? `Searching in ${manualCity}...`
                  : searchType === "coordinates"
